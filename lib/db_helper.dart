...
class DBHelper{
  ...
  _onCreate(Database db,int version) async{
    await db.execute("CREATE TABLE $DURATION_TABLE ($ID INTEGER PRIMARY KEY, $NAME TEXT, $DURATION INTEGER, $TASK_DATE INTEGER DEFAULT (cast(strftime('%s','now') as int)))");
    await db.execute("CREATE TABLE $START_END_TABLE ($ID INTEGER PRIMARY KEY, $NAME TEXT, $START_TIME INTEGER, $END_TIME INTEGER, $TASK_DATE INTEGER DEFAULT (cast(strftime('%s','now') as int)))");
  }
  ...
  Future<List<DurationTask>> getDurationTasks() async{
     var dbClient=await database;
     List<Map>maps=await dbClient.query(DURATION_TABLE, columns:[ID,NAME,DURATION,TASK_DATE],
  ...
  Future<List<StartEndTask>> getStartEndTasks() async{
     var dbClient=await database;
     List<Map>maps=await dbClient.query(START_END_TABLE, columns:[ID,NAME,START_TIME,END_TIME,TASK_DATE],
     orderBy: '$TASK_DATE DESC');
  ...
  Future<int>deleteDurationTask(int id) async{
    print('task id $id');
    var dbClient=await database;
    return await dbClient.delete(DURATION_TABLE,where:'$ID = ?',whereArgs: [id]);
  }

  Future<int>deleteStartEndTask(int id) async{
    var dbClient=await database;
    return await dbClient.delete(START_END_TABLE,where:'$ID = ?',whereArgs: [id]);
  }

  Future<int>updateDurationTask(DurationTask task) async{
    var dbClient=await database;
    return await dbClient.update(DURATION_TABLE, task.toMap(), where:'$ID= ?',whereArgs:[task.id]);
  }

  Future<int>updateStartEndTask(StartEndTask task) async{
    var dbClient=await database;
   return await dbClient.update(START_END_TABLE, task.toMap(), where:'$ID= ?',whereArgs:[task.id]);
  }
